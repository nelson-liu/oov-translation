...
class YFOptimizer(object):
    ...
    def curvature_range(self):
        ...
        global_state["h_min_avg"] = \
            global_state["h_min_avg"] * beta + \
            (1 - beta) * torch.min(curv_win[:valid_end])
        global_state["h_max_avg"] = \
            global_state["h_max_avg"] * beta + \
            (1 - beta) * torch.max(curv_win[:valid_end])
    ...
    def dist_to_opt(self):
        ...
        global_state["grad_norm_avg"] = \
            global_state["grad_norm_avg"] * beta + \
            (1 - beta) * math.sqrt(global_state["grad_norm_squared"])
        global_state["dist_to_opt_avg"] = \
            global_state["dist_to_opt_avg"] * beta \
            + (1 - beta) * global_state["grad_norm_avg"] / \
    ...
    def after_apply(self):
        ...
        global_state['grad_norm_squared_avg'] = \
            global_state['grad_norm_squared_avg'] * beta + \
            (1 - beta) * global_state['grad_norm_squared']
    ...
    def get_mu(self):
        ...
        roots = np.roots(coef)
        root = roots[np.logical_and(
            np.logical_and(np.real(roots) > 0.0, np.real(roots) < 1.0),
            np.imag(roots) < 1e-5)]
        assert root.size == 1
        dr = self._h_max / self._h_min
        self._mu_t = max(
            np.real(root)[0]**2, ((np.sqrt(dr) - 1) / (np.sqrt(dr) + 1))**2)
        return
